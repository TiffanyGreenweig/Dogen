...
// todo 支持内容html渲染
export const ChatFrame = ({ comments, defaultMsg, commentRef }: { comments: IChatItem[], defaultMsg?: string, commentRef?: any }) => {
  ...
  return (
    <div className="chat-comment-wrapper">
      <div className="chat-comment-content">
        <ChatItem props={{
          content: defaultMsg || '您好，请问你有什么困惑呢？',
        ...
        <div id="scrollSetBar" className="scroll-set-bar">
          {updateLoading && <Spin className="chat-comment-loading loading-circle" indicator={<i className="icon-loading" />} />}
        </div>
      </div>
    </div>
  );
